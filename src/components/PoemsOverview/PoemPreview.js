...
function PoemPreview({poem, linkPrefix}) {
    ...
    return <div>
        <Link to={linkPrefix + '/' + poem.id}>
            <Tilt tiltEnable={userCanHover && !poem.completed} tiltReverse={true} tiltMaxAngleX={MAX_TILT_ANGLE}
                  tiltMaxAngleY={MAX_TILT_ANGLE}
                ...
                <div className="poem-preview-content">
                    <p>
                        {poem.lines && poem.lines.slice(0, PREVIEW_LINES_NUM).map((line, idx) =>
                            <li className={"poem-preview-line" + (line.creator === user.nickname ? ' personal' : '')}
                                key={"my-preview-" + idx}
                                style={{
